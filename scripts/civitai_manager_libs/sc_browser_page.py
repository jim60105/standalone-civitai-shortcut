...
def get_thumbnail_list(
    ...
    total = 0
    max_page = 1
    shortcut_list = ishortcut.get_image_list(
...
def on_shortcut_gallery_refresh(
    sc_types, sc_search, sc_basemodels, sc_classifications, show_downloaded_sc, columns, rows
):
    thumb_list, thumb_totals, thumb_max_page = get_thumbnail_list(
        sc_types, show_downloaded_sc, sc_search, sc_basemodels, sc_classifications, 1, columns, rows
    )
    return (
        gr.update(value=thumb_list),
        gr.update(
