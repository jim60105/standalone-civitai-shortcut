        ...
        if items.get("Software", None) == "NovelAI":
            try:
                ...
                geninfo = (
                    ...
                    f"Steps: {json_info['steps']}, Sampler: {sampler}, "
                    f"CFG scale: {json_info['scale']}, Seed: {json_info['seed']}, "
                    f"Size: {image.width}x{image.height}, Clip skip: 2, ENSD: 31337"
                )
            except Exception:
...
""".strip()
    ...
    def _extract_additional_info(
        self, metadata: Dict[str, str], parameters_text: str
    ) -> Optional[str]:
    ...
    def parse_generation_parameters(self, x: str, skip_fields: List[str] = None) -> Dict[str, Any]:
        ...
        This implementation exactly matches AUTOMATIC1111's
        infotext_utils.parse_generation_parameters.
        """
        if not x:
            return {}
    ...
    def _extract_advanced_parameters(self, text: str) -> Dict[str, Any]:
        ...
        if embedding_matches:
            ...
            embeddings = [
                emb for emb in embedding_matches if not any(lora[0] in emb for lora in lora_matches)
            ]
            if embeddings:
    ...
    def extract_prompt_from_parameters(self, parameters_text: str) -> Tuple[str, str]:
        ...
        for line in lines:
            ...
            if line_lower.startswith("negative prompt:"):
                ...
                start_pos = line.lower().find("negative prompt:") + len("negative prompt:")
                negative_content = line[start_pos:].strip()
                if negative_content:
    ...
    def format_parameters_for_display(self, parameters: Dict[str, Any]) -> str:
        ...
        for key, value in parameters.items():
            if key not in ["prompt", "negative_prompt"] + param_order and value is not None:
                formatted_key = self._format_parameter_key_for_display(key)
    ...
    def _format_parameter_value_for_display(self, key: str, value: Any) -> str:
        ...
        # Handle lists (for LoRA, embeddings, etc.)
            ...
            if key == "lora":
                return ", ".join(
                    [
                        f"<lora:{item['name']}:{item['strength']}>"
                        for item in value
