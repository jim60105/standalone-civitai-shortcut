...
class WebUISamplerProvider(ISamplerProvider):
    ...
    def get_samplers_for_img2img(self) -> List[str]:
        ...
        try:
            ...
            return [x.name for x in samplers_for_img2img] if samplers_for_img2img else self._get_default_samplers()
        except (ImportError, AttributeError):
            return self._get_default_samplers()
    ...
    def _get_default_upscale_modes(self) -> List[str]:
        ...
        return [
            "Linear", "Bilinear", "Bicubic", "Nearest", "Area", "Lanczos"
        ]
    
    def _get_default_sd_upscalers(self) -> List[str]:
        """Get default SD upscalers."""
        return [
            "None", "Lanczos", "Nearest", "LDSR", "BSRGAN", "ESRGAN_4x",
            "R-ESRGAN 4x+", "R-ESRGAN 4x+ Anime6B", "ScuNET GAN", 
