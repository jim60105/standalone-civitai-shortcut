...
from .interfaces import (
    IPathManager, IConfigManager, IMetadataProcessor, 
    IUIBridge, ISamplerProvider, IParameterProcessor
)
...
class CompatibilityLayer:
    ...
    def _create_path_manager(self) -> IPathManager:
        ...
        if self.mode == 'webui':
            ...
            from .standalone_adapters.standalone_path_manager import StandalonePathManager
    ...
    def _create_config_manager(self) -> IConfigManager:
        ...
        if self.mode == 'webui':
            ...
            from .standalone_adapters.standalone_config_manager import StandaloneConfigManager
    ...
    def _create_metadata_processor(self) -> IMetadataProcessor:
        ...
        if self.mode == 'webui':
            ...
            from .standalone_adapters.standalone_metadata_processor import StandaloneMetadataProcessor
    ...
    def _create_sampler_provider(self) -> ISamplerProvider:
        ...
        if self.mode == 'webui':
            ...
            from .standalone_adapters.standalone_sampler_provider import StandaloneSamplerProvider
    ...
    def _create_parameter_processor(self) -> IParameterProcessor:
        ...
        if self.mode == 'webui':
            from .webui_adapters.webui_parameter_processor import WebUIParameterProcessor
            return WebUIParameterProcessor()
        else:
            from .standalone_adapters.standalone_parameter_processor import StandaloneParameterProcessor
...
def get_compatibility_layer(mode: Optional[EnvironmentType] = None) -> CompatibilityLayer:
    """
