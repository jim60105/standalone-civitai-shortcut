...
from .gradio_compat import SelectData, State, Gallery, File, Dropdown
...
def on_ui(recipe_input, shortcut_input, civitai_tabs):
        ...
        with gr.Tabs() as civitai_shortcut_tabs:
            with gr.TabItem("Register Model"):
                ...
                with gr.Row():
                    with gr.Column():
                        gr.Markdown(
                            value="Using the model URL from the Civitai site, you register the information of the model. You can click and drag the Civitai Model's URL or drag and drop a saved internet shortcut. Additionally, you can select multiple internet shortcuts and drop them all at once.",
                            visible=True,
                        )
            ...
            with gr.TabItem("Scan New Version"):
                with gr.Row():
                    with gr.Column():
                        shortcut_new_version_type = Dropdown(
                        ...
                        sc_new_version_gallery = Gallery(
                        ...
                        gr.Markdown(
                            value="The feature is to search for new versions of models on Civitai among the downloaded ones.",
                            visible=True,
                        )
            with gr.TabItem("NSFW Filter"):
                with gr.Row():
                    with gr.Column():
                        nsfw_filter_enable = Dropdown(
                            value='On',
                            choices=['On', 'Off'],
    ...
    refresh_NSFW.change(
        fn=on_refresh_NSFW_change, inputs=None, outputs=[nsfw_filter_enable, nsfw_level]
    )
...
def on_sc_gallery_select(evt: SelectData):
    ...
    if evt.value:
        shortcut = evt.value
        sc_model_id = setting.get_modelid_from_shortcutname(
            shortcut
        )  # shortcut[0:shortcut.find(':')]
