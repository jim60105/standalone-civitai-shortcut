...
def on_ui(shortcut_input):
    ...
    with gr.Column(scale=setting.shortcut_browser_screen_split_ratio):
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Additional Shortcut Models"):
                sc_gallery, refresh_sc_browser, refresh_sc_gallery = (
                    classification_browser_page.on_ui(classification_shortcuts, False)
                )
                ...
                with gr.Column():
                    ...
                    with gr.Tabs() as classification_information_tabs:
                        with gr.TabItem("Classification Shortcuts", id="Classification_Shortcuts"):
                            with gr.Row():
                                with gr.Column(scale=4):
                                    ...
                                    with gr.Row():
                                        classification_prevPage_btn = gr.Button(
                                            value="Prev", scale=1
                                        )
                                        classification_nextPage_btn = gr.Button(
                                            value="Next", scale=1
                                        )

                                    classification_shortcut_delete = gr.Checkbox(
                                        label="Delete from classification when selecting a thumbnail.",
                                        value=False,
                                    )
                                ...
                                with gr.Accordion(
                                ...
                                ) as shortcut_model_information:
                                    shortcut_modelid = gr.Textbox(visible=False)
                                    shortcut_modeltype = gr.Textbox(
                                    ...
                                    delete_shortcut_model_btn = gr.Button(
                                        value="Delete", variant="primary"
                                    )
                                    close_shortcut_model_information_btn = gr.Button(
                        ...
                        with gr.TabItem("Classification Info"):
                            classification_info = gr.Textbox(
                                label="Description", value="", interactive=True, lines=7
                            )
                    with gr.Row():
                        classification_create_btn = gr.Button(value="Create", variant="primary")
                        classification_update_btn = gr.Button(
    ...
    classification_gallery_page.release(
        ...
        inputs=[classification_gallery_page],
        outputs=[refresh_gallery],
    )
    ...
    refresh_classification.change(
        ...
        inputs=[selected_classification_name],
        outputs=[
            classification_name,
            classification_info,
            refresh_sc_browser,
    ...
    refresh_gallery.change(
        ...
        inputs=[classification_shortcuts, classification_gallery_page],
        outputs=[classification_gallery, classification_gallery_page, refresh_sc_browser],
        show_progress=False,
    )
    ...
    classification_gallery.select(
        ...
        inputs=[classification_shortcuts, classification_shortcut_delete],
        outputs=[
            classification_shortcuts,
            refresh_gallery,
    ...
    classification_reload_shortcut_btn.click(
        ...
        inputs=[selected_classification_name],
        outputs=[classification_shortcuts, classification_gallery_page, refresh_gallery],
    )
    ...
    classification_prevPage_btn.click(
        ...
        inputs=[classification_gallery_page],
        outputs=[refresh_gallery, classification_gallery_page],
        show_progress=False,
    )
...
def on_classification_new_btn_click():
    ...
    return (
        ...
        gr.update(value=""),
        None,
        gr.update(value=1, minimum=1, maximum=1, step=1, label=f"Total {1} Pages"),
        None,
        current_time,
        current_time,
...
def on_sc_gallery_select(evt: gr.SelectData, shortcuts, page):
    ...
    if evt.value:
        ...
        return (
            shortcuts,
            gr.update(value=page, maximum=page),
            current_time,
            gr.update(selected="Classification_Shortcuts"),
        )
    return shortcuts, gr.update(value=page), None, gr.update(selected="Classification_Shortcuts")


@with_error_handling(
    fallback_value=(gr.update(value=None), gr.update(minimum=1, value=1, maximum=1, step=1, label=f"Total {1} Pages"), datetime.datetime.now()),
...
def on_classification_gallery_loading(shortcuts, page=0):
    ...
    if shortcuts:
        ...
        # 현재 표시될 페이지 양만 잘라준다.
        shortcuts, totals, max_page, cur_page = paging_classification_shortcuts_list(
            shortcuts, page
        )
    ...
    current_time = datetime.datetime.now()
    return (
        gr.update(value=result_list),
        gr.update(
            minimum=1, value=cur_page, maximum=max_page, step=1, label=f"Total {max_page} Pages"
        ),
        current_time,
    )
...
@with_error_handling(
    fallback_value=(gr.update(value=""), gr.update(choices=[]), datetime.datetime.now(), gr.update(visible=True), gr.update(visible=True), gr.update(visible=False)),
...
def on_classification_create_btn_click(new_name, new_info, classification_shortcuts):
    ...
    if classification.create_classification(new_name, new_info):
        ...
        return (
            ...
            gr.update(choices=classification.get_list(), value=new_name),
            current_time,
            gr.update(label=new_name),
            gr.update(visible=False),
            gr.update(visible=True),
        )
    return (
...
@with_error_handling(
    fallback_value=(gr.update(value=""), gr.update(choices=[]), datetime.datetime.now(), gr.update(label="")),
    exception_types=(FileOperationError, ValidationError),
    retry_count=1,
...
def on_classification_update_btn_click(select_name, new_name, new_info, classification_shortcuts):
    ...
    return (
        ...
        gr.update(choices=classification.get_list(), value=chg_name),
        current_time,
...
@with_error_handling(
    fallback_value=(gr.update(value=""), gr.update(choices=[]), datetime.datetime.now(), gr.update(label=setting.NEWCLASSIFICATION), gr.update(visible=True), gr.update(visible=False)),
