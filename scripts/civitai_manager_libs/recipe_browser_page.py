...
def on_ui():
    ...
    with gr.Accordion(label="Filter Reference Shortcut Items", open=False):
        ...
        recipe_reference_gallery_page = gr.Slider(
            minimum=1,
            maximum=reference_max_page,
            value=1,
            step=1,
            label=f"Total {reference_max_page} Pages",
            interactive=True,
            visible=True,
        )
        with gr.Row():
    ...
    refresh_recipe_reference_select_gallery.change(
        ...
        outputs=[recipe_reference_select_gallery, refresh_recipe_search],
        show_progress=False,
    )
    ...
    recipe_reference_gallery_page.release(
        ...
        inputs=[recipe_reference_gallery_page],
        outputs=[
    ...
    refresh_recipe_search.change(
        ...
        inputs=[recipe_search, recipe_classification_list, recipe_reference_select],
        outputs=[recipe_gallery, recipe_gallery_page],
    )
    ...
    recipe_reference_prevPage_btn.click(
        ...
        inputs=[recipe_reference_gallery_page],
        outputs=[recipe_reference_gallery, recipe_reference_gallery_page],
        show_progress=False,
    )
...
def on_recipe_gallery_page(search, classification, shortcut, page=0):
    thumb_list, thumb_totals, thumb_max_page = get_recipe_list(
        search, classification, shortcut, page
    )
...
def on_recipe_list_search(search, classification, shortcut):
    ...
    return gr.update(value=thumb_list), gr.update(
        minimum=1, maximum=thumb_max_page, value=1, step=1, label=f"Total {thumb_max_page} Pages"
    )
...
def on_refresh_recipe_browser_change(search, classification, shortcut, sc_page, rs_page):
    ...
    thumb_list, thumb_totals, thumb_max_page = get_recipe_list(
        search, classification, shortcut, sc_page
    )
    ...
    return (
        ...
        gr.update(value=reference_list),
        gr.update(
            minimum=1,
            maximum=reference_max_page,
            value=rs_page,
    ...
    if shortcuts:
        ...
        for mid in shortcuts:
            if str(mid) in ISC.keys():
                v = ISC[str(mid)]
                if ishortcut.imageprocessor.is_sc_image(v['id']):
                    if bool(v['nsfw']) and settings.get_setting('nsfw_filter_enable'):
                        result_list.append(
                            (
                                settings.get_nsfw_disable_image(),
                                settings.set_shortcutname(v['name'], v['id']),
