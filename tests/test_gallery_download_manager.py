...
def download_manager():
    ...
    with patch('scripts.civitai_manager_libs.gallery.download_manager.get_http_client') as mock_get_client:
...
def test_download_images_batch_success(download_manager: GalleryDownloadManager):
    ...
    with patch('os.path.isfile', return_value=False), \
         patch('scripts.civitai_manager_libs.gallery.download_manager.settings.get_setting', return_value=2):
        # Act
        result = download_manager.download_images_batch(dn_image_list)
...
def test_download_user_gallery_success(download_manager: GalleryDownloadManager):
    ...
    with patch('scripts.civitai_manager_libs.ishortcut_core.model_processor.ModelProcessor') as mock_model_processor, \
         patch('scripts.civitai_manager_libs.util.make_download_image_folder', return_value="/tmp/TestModel") as mock_make_folder, \
         patch('os.path.exists', return_value=True), \
         patch('shutil.copyfile'), \
         patch('scripts.civitai_manager_libs.gallery.download_manager.ParallelImageDownloader') as mock_downloader:
...
def test_load_gallery_images_success(download_manager: GalleryDownloadManager):
    '''Test successful loading of gallery images.'''
    # Arrange
    images_url = ["http://example.com/image1.jpg"]
    progress_mock = MagicMock()
    progress_mock.tqdm.return_value = images_url

    with patch.object(download_manager, 'download_single_image', return_value=True) as mock_download:
        # Act
...
def test_preload_next_page_disabled(download_manager: GalleryDownloadManager):
    ...
    # Arrange
    with patch('scripts.civitai_manager_libs.gallery.download_manager.settings.usergallery_preloading', False):
        # Act
...
def test_preload_next_page_success(download_manager: GalleryDownloadManager):
    ...
    # Arrange
    with patch('scripts.civitai_manager_libs.gallery.download_manager.settings.usergallery_preloading', True), \
        ...
        # Act
        download_manager.preload_next_page("some_url", {'totalPageUrls': ["url1", "url2"], 'nextPage': 'url2'})
...
def test_download_images_parallel_success(download_manager: GalleryDownloadManager):
    ...
    with patch('scripts.civitai_manager_libs.gallery.download_manager.ParallelImageDownloader') as mock_downloader:
